...
**komadown** includes a few font packs that consist of font combinations 
(serif, sans-serif, and monospace). All are based on the
...
times[^1]            Times New Roman    Helvetica      newtxtt


[^1]: There is currently an issue using **authblk** (which **komadown**
uses to set up title blocks) together with **newtxtext** that causes
superscripts for the author footnotes to be malformed.
...
The default bibliography style is the vancouver [@patrias_2007] style. It
can be replaced with any `.csl` style using the usual **pandoc** interface.
